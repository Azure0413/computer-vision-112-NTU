...
def search_motion(ref: np.ndarray, target: np.ndarray, seg_ref: h5py.Dataset, seg_target: h5py.Dataset, ref_id, target_id):
    orb = cv2.ORB_create(nfeatures=10000, nlevels=8, scaleFactor=1.2)

    feature_img = np.ones((ref.shape[0], ref.shape[1]), dtype=np.uint8) * 255
    all_keypoints = []
    all_descriptors = []
    for w in range(3):
        for h in range(3):
            mask = np.zeros_like(ref, dtype=np.uint8)
            mask[h*ref.shape[0]//3:(h+1)*ref.shape[0]//3, w*ref.shape[1]//3:(w+1)*ref.shape[1]//3] = 255

            keypoints_tar, descriptors_tar = orb.detectAndCompute(target, mask=mask)
            keypoints_ref, descriptors_ref = orb.detectAndCompute(ref, mask=mask)
            all_keypoints.extend(keypoints_ref)
            if descriptors_ref is not None:
                if len(all_descriptors) == 0:
                    all_descriptors = descriptors_ref
                else:
                    all_descriptors = np.vstack((all_descriptors, descriptors_ref))

            # Draw matches as blud and red dot on a white image
            feature_img = cv2.drawKeypoints(feature_img, keypoints_ref, feature_img, color=(0, 0, 255))
            feature_img = cv2.drawKeypoints(feature_img, keypoints_tar, feature_img, color=(255, 0, 0))
            cv2.imwrite(f"matches/{target_id:03d}_{ref_id:03d}.png", feature_img)
...
for target, ref0, ref1 in process_order:
    ...
    Hs_ref0 = search_motion(frames[ref0], frames[target], seg_ref0, seg_target, ref0, target)
    Hs_ref1 = search_motion(frames[ref1], frames[target], seg_ref1, seg_target, ref1, target)
...
if save:
    ...
    save_results(process_order, final_compensated, final_mask, "solution_masked", apply_mask=True)
    print("Results saved")