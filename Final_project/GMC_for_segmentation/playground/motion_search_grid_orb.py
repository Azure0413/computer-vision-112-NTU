    ...
    for grid_size in range(1, 4):
        ...
        for w in range(grid_size):
            for h in range(grid_size):
                ...
                all_keypoints_tar.extend(keypoints_tar)
                if descriptors_tar is not None:
                    if len(all_descriptors_tar) == 0:
                        all_descriptors_tar = descriptors_tar
                    else:
                        all_descriptors_tar = np.vstack((all_descriptors_tar, descriptors_tar))
                
                all_keypoints_ref.extend(keypoints_ref)
                if descriptors_ref is not None:
                    if len(all_descriptors_ref) == 0:
                        all_descriptors_ref = descriptors_ref
                    else:
                        all_descriptors_ref = np.vstack((all_descriptors_ref, descriptors_ref))

        bf = cv2.BFMatcher(cv2.NORM_HAMMING2, crossCheck=True)
        matches = bf.match(all_descriptors_ref, all_descriptors_tar)

        for i, seg in enumerate(seg_ref):
            clusters = []
            for m in matches:
                if seg[int(all_keypoints_ref[m.queryIdx].pt[1]), int(all_keypoints_ref[m.queryIdx].pt[0])]:
                    clusters.append(m)
            points_ref = np.array([all_keypoints_ref[m.queryIdx].pt for m in clusters])
            points_tar = np.array([all_keypoints_tar[m.trainIdx].pt for m in clusters])
