...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    train_mean = np.mean(train_img_feats, axis=0)
    train_std = np.std(train_img_feats, axis=0)
    train_img_feats_normalized = (train_img_feats - train_mean) / (train_std + 1e-6)
    test_img_feats_normalized = (test_img_feats - train_mean) / (train_std + 1e-6)

    test_predicts = []
    p_value = 0.5
    dists = cdist(test_img_feats_normalized, train_img_feats_normalized, metric='minkowski', p=p_value)
    
    K = min(6, len(set(train_labels)))
    
    for i in range(len(test_img_feats_normalized)):
