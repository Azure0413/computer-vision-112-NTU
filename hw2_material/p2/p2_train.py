...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    # is not fixed.                                                #
    ################################################################
    train_acc = result_lists['train_acc']
    train_loss = result_lists['train_loss']
    val_acc = result_lists['val_acc']
    val_loss = result_lists['val_loss']
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for data in val_loader:
                ...
                val_loss += loss.item()
                val_correct += torch.sum(torch.argmax(output, dim=1) == labels)
            ######################### TODO End ##########################
